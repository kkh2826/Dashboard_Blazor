...
namespace UploadApp.Controllers
{
    public class UploadDownloadController : Controller
    {
        ...
        public async Task<IActionResult> BoardDown(int id)
        {
            ...
            else
            {
                if (!string.IsNullOrEmpty(model.FileName))
                {
                    ...
                    {
                        byte[] fileBytes = await _fileStorageManager.DownloadAsync(model.FileName, Path.Combine(_environment.WebRootPath, "files") + "\\");
